#baseline 3: model with variate window size and without LSTM
...
class Train():
    ...
    def Train_Test(self)->float:
        ...
        for epoch in range(1, self.arg.epoch + 1):
                ...
                train_dataloader = self.data.getTrainDataloader(
                    window_size=self.arg.accept_window,
                ...
                print('Epoch: {:03d}, '
                      ...
                      'Test RMSE: {:.4f}, '
                      'Test Score: {:.4f}, '
                      'training Window Size: {}'.format(epoch, train_loss, test_RMSE, test_score,
                                                        self.arg.accept_window))
                ...
                if test_RMSE < test_RMSE_best:
                    ...
                    self.save_RMSE(test_RMSE, epoch)
    ...
    def save_RMSE(self, RMSE, epoch):
        rmse = round(RMSE, 3)
        file_dir = os.path.join('./param_model',self.arg.dataset)
        # file__list = os.listdir(file_dir)
        new_file = os.path.join(file_dir, 'window_size' + str(self.arg.accept_window) + '.txt')
        with open(new_file, mode='a') as file:
            file.write(f'Epoch: {epoch}, Window Size: {self.arg.accept_window}, RMSE: {rmse}\n')
...
def args_config(window_size, dataset_choice : int) -> Namespace:
    ...
    match dataset_choice:
        ...
        case 2:
            # parser.add_argument('accept_window', type=int, default=50)
            arguments.accept_window = window_size
...
if __name__ == '__main__':
    window_size_variant = (50, 60, 70, 80, 90, 100, 110, 120)
    for window_size in window_size_variant:
        main(window_size)
