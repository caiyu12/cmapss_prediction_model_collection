...
#  of the dataset.
class TrainEngineDataset(Dataset):
    def __init__(
            self,
    ...
    def __getitem__(self, index : int) -> (Tensor, Tensor):
        ...
        data_tensor  = self.data_list[0][index_slided:index_slided+self.window_size, 1:]
        target_tensor = self.targ_list[0][index_slided+self.window_size-1, 1:]

        return data_tensor, target_tensor


class TestEngineDataset(Dataset):
    def __init__(
            self,
            data_group : DataFrameGroupBy,
            targ_group : DataFrameGroupBy,
