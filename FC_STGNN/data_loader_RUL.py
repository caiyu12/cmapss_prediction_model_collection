...
class CMPDataIter(data.IterableDataset):
    ...
    def __init__(self, data_root, data_set, max_rul, seq_len, net_name):
        ...
        # REMIND: Here comes data regulation
    ...
    def _get_data(self, data_root, data_set):
        ...
        train_data_df.drop(train_data_df.columns[[26, 27]], axis=1, inplace=True) # FIXME: wrong csv reading
        train_data_df.columns = self.column_names
        train_data_df = train_data_df.sort_values(
            by = ['id','cycle'],
            inplace=False, # When inplace = True , it will not return a new object, instead it will modify the original object
            ascending=True
        ) # REMIND: "by=" defines sort targets, the former has higher priority.
    ...
    def _process(self, train_df, test_df, test_truth):
        ...
        # process train data
        train_rul = pd.DataFrame(data=train_df.groupby('id')['cycle'].max()).reset_index()
        ...
        train_engine_num : int = train_df['id'].nunique()
        logging.info("CMPDataIter:: iterator initialized (train engine number: {:})".format(train_engine_num))

        # process test data
        test_rul = pd.DataFrame(test_df.groupby('id')['cycle'].max()).reset_index(drop=False)
        ...
        for train_idx, train in grouped_train:
            ...
            scaled_train = scaler.fit_transform(train.iloc[:, 3:]) # only normalize the sensor data
        ...
        # generate final data:
        #generate self.seq_len x 14 windows to obtain train_x
        seq_gen = []
        start_index = 0
        for i in range(train_engine_num):
            end_index = start_index+train_rul.loc[i, 'max'] # row: i, column: 'max'
            if end_index - start_index < self.seq_len-1:
                print('train data less than seq_len!')
            # for sensor train matrix, number of self.seq_len X 14 needed per data points (minus the first sequence length) per engine, so the array input start from start index
            val=list(self.gen_sequence(train_normalized.iloc[start_index:end_index, :], self.seq_len, train_normalized.columns))
            seq_gen.extend(val)
            start_index = end_index
        train_x = seq_gen

        #generate self.seq_len x 4 windows to obtain train_ops
        seq_gen = []
        start_index = 0
        for i in range(train_engine_num):
            end_index = start_index+train_rul.loc[i, 'max']
            # print(end_index)
            # for ops train matrix, number of self.seq_len X 4 needed per data points (minus the first sequence length) per engine, so the array input start from start index
        ...
        for i in range(test_engine_num):
            ...
            #diff@xuqing
            # for test matrix, only 1 of self.seq_len X 14 needed per engine, so the array input start from end index - sequence length
        ...
        for i in range(test_engine_num):
            ...
            if end_index - start_index < self.seq_len:
                val=list([self.gen_test_labels(test_y.iloc[start_index:end_index, :], self.seq_len, test_y.columns)])
            else:
    ...
    def gen_labels(self, id_df, seq_length, label):
        ...
        # ...
        # [200]]
        data_matrix = id_df[label].values # change Dataframe into np.ndarray
        num_elements = data_matrix.shape[0]
        label_matrix = []
        for i in range(num_elements-(seq_length-1)):
            yield data_matrix[i+(seq_length-1), :] # SOLVED: label error(0:29 -> 29)
...
class MyCMPDataIter(CMPDataIter_graph):
    ...
    # Implement any other abstract methods from CMPDataIter_graph here